...
const LAPORAN_DATA = {
    ...
    'row-3': { kode:'KRF-230226-T5U6', nama:'Yoga Pratama',     nis:'12345', kelas:'XII AKL-2',  urgensi:'rendah', email:'yoga.p@student.smkm3.sch.id',    tanggal:'23 Feb 2026', tempat:'Lorong Kelas',      pelaku:'Teman Sekelas',          korban:'Yoga Pratama',     saksi:'Tidak ada',      deskripsi:'Teman saya sering menggunakan nama saya untuk hal-hal yang tidak baik dan membuat nama saya jelek di lingkungan sekolah.' },
    'row-4': { kode:'KRF-220226-U7V8', nama:'Hani Pertiwi',     nis:'10678', kelas:'X AKL-2',    urgensi:'tinggi', email:'hani.p@student.smkm3.sch.id',    tanggal:'22 Feb 2026', tempat:'Ruang Kelas',       pelaku:'Teman Sebangku',         korban:'Hani Pertiwi',     saksi:'Guru Mapel',     deskripsi:'Saya dipaksa memberikan jawaban ujian secara berulang kali dengan ancaman akan disebarkan aib saya.' },
    'row-5': { kode:'KRF-210226-W9X0', nama:'Fajar Setiawan',   nis:'11890', kelas:'XI TKJ-2',   urgensi:'sedang', email:'fajar.s@student.smkm3.sch.id',   tanggal:'21 Feb 2026', tempat:'Parkiran Sekolah',  pelaku:'Siswa Kelas XII',        korban:'Fajar Setiawan',   saksi:'Tidak ada',      deskripsi:'Motor saya sering diganggu dan kunci helm hilang beberapa kali oleh orang yang sama.' },
    'row-6': { kode:'KRF-200226-Y1Z2', nama:'Novi Anggraini',   nis:'12890', kelas:'XII RPL-2',  urgensi:'rendah', email:'novi.a@student.smkm3.sch.id',    tanggal:'20 Feb 2026', tempat:'Perpustakaan',      pelaku:'Teman Sekelas',          korban:'Novi Anggraini',   saksi:'Petugas Pustaka', deskripsi:'Buku-buku perpustakaan yang saya pinjam sering diambil paksa dan tidak dikembalikan oleh teman.' },
};
...
function renderTable() {
    ...
    if (_filteredRows.length === 0) {
        ...
        tbody.innerHTML = pageRows.map((rowId, idx) => {
            const d=LAPORAN_DATA[rowId]; const av=avatarColor(idx);
            return `
...
function updateTableInfo() {
    ...
    const s=(_currentPage-1)*PER_PAGE+1, e=Math.min(_currentPage*PER_PAGE,_filteredRows.length);
...
function renderPagination() {
    ...
    const total=Math.ceil(_filteredRows.length/PER_PAGE); wrap.innerHTML='';
    if(total<=1) return;
...
function applyFilter(){
    ...
    _filteredRows=_allRows.filter(id=>{const d=LAPORAN_DATA[id];return(!q||d.kode.toLowerCase().includes(q)||d.nama.toLowerCase().includes(q)||d.nis.includes(q))&&(!u||d.urgensi===u);});
...
function handleQuickAction(action,rowId){const d=LAPORAN_DATA[rowId];if(!d)return;_currentRow=document.getElementById(rowId);_currentData=d;document.getElementById('modalDetail').style.display='none';document.body.style.overflow='';triggerKonfirmasi(action);}
