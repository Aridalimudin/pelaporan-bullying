<aside class="admin-sidebar" id="adminSidebar">
    ...
    <div class="sidebar-brand">
        <div class="sidebar-brand-icon">
            <img src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path('images/logoSMK.png'))) }}"
                alt="Logo SMK"
                style="width:42px;height:42px;object-fit:contain;">
        </div>
        <div class="sidebar-brand-text">
            <span class="brand-name">SIP Bullying</span>
            <span class="brand-sub">SMK Muhammadiyah 3</span>
        </div>
    ...
    <nav class="sidebar-nav">
        ...
        {{-- PELAPORAN — tampil jika punya view-laporan --}}
        @if($user->hasPermission('view-laporan'))
        ...
        {{-- ADMINISTRASI — tampil jika punya salah satu permission admin --}}
        @if($user->hasPermission('manage-master-data') || $user->hasPermission('manage-user') || $user->hasPermission('manage-role') || $user->hasPermission('manage-permission'))
        <div class="nav-section-label">Administrasi</div>

            {{-- Master Data --}}
            @if($user->hasPermission('manage-master-data'))
            ...
            {{-- Manajemen User --}}
            @if($user->hasPermission('manage-user') || $user->hasPermission('manage-role') || $user->hasPermission('manage-permission'))
            <div class="nav-group {{ in_array($activePage ?? '', ['daftar-users','daftar-roles','daftar-permissions']) ? 'open' : '' }}" id="groupUser">
                ...
                <div class="nav-group-children">
                    @if($user->hasPermission('manage-user'))
                    ...
                    @endif
                    @if($user->hasPermission('manage-role'))
                    ...
                    @endif
                    @if($user->hasPermission('manage-permission'))
        ...
        {{-- ANALITIK — tampil jika punya view-rekap --}}
        @if($user->hasPermission('view-rekap'))
        <div class="nav-section-label">Analitik</div>
        <div class="nav-group {{ in_array($activePage ?? '', ['rekap-bulan','rekap-semester','rekap-tahun']) ? 'open' : '' }}" id="groupAnalitik">
            ...
            <div class="nav-group-children">
                ...
                <a href="{{ route('administrator.yearly') }}" class="nav-child {{ ($activePage ?? '') === 'rekap-tahun' ? 'active' : '' }}">
                    <svg class="child-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                    </svg>
                    <span>Per Tahun</span>
    ...
    <div class="sidebar-footer">
        <div class="sidebar-profile">
            <div class="sidebar-profile-avatar">{{ strtoupper(substr(auth()->user()->nama ?? 'A', 0, 1)) }}</div>
            <div class="sidebar-profile-info">
                <span class="sidebar-profile-name">{{ auth()->user()->nama ?? 'Admin' }}</span>
                <span class="sidebar-profile-role">{{ auth()->user()->getPrimaryRole()?->nama ?? '-' }}</span>
            </div>
        </div>
...
<style>
    ...
    .sidebar-brand-icon {
        width: 42px;
        height: 42px;
        background: transparent; /* hapus background hijau */
