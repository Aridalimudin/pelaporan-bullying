                    ...
                    <div class="md-grid2">
                        ...
                        <div class="md-f">
                            ...
                            <div id="mdPelaku" style="padding:8px 11px;background:#fef2f2;
                                border:1.5px solid #fecaca;border-radius:9px;min-height:38px;">—</div>
                        </div>
                        <div class="md-f">
                            <div class="md-fl">KORBAN</div>
                            <div id="mdKorban" style="padding:8px 11px;background:white;
                                border:1.5px solid #e5e7eb;border-radius:9px;min-height:38px;">—</div>
                        </div>
                        <div class="md-f">
    ...
    function openDetailModal(data, type, rowEl, scrollToTindak = false) {
        ...
        if (!isLM) {
            ...
            if (elJam) elJam.innerHTML = data.jam && data.jam !== '-'
                ? `<span>${data.jam}</span>`
                : `<span style="color:#9ca3af;font-style:italic;font-size:.78rem;">Tidak diisi</span>`;
        ...
        if (data.files && data.files.length > 0) {
            data.files.forEach((file) => {
                ...
                let content = '';
                if (isImage) {
                    content = `<img src="${file.url}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;">`;
                } else if (isVideo) {
                    content = `<div style="display:flex; flex-direction:column; align-items:center; gap:4px;">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:24px; color:#ef4444;"><path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                <span style="font-size:10px; font-weight:bold; color:#374151;">VIDEO</span>
                            </div>`;
                }

                const htmlBukti = `
                    <div class="md-bukti" onclick="window.open('${file.url}', '_blank')" 
                        style="cursor:pointer; border:1.5px solid #e5e7eb; background:#fff; overflow:hidden; position:relative;">
                        ${content}
                    </div>`;
